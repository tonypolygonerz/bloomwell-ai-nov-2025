...
export class OllamaCloudClient {
  ...
  constructor(config?: OllamaCloudConfig) {
    this.apiKey =
      config?.apiKey || process.env.OLLAMA_CLOUD_API_KEY || process.env.OLLAMA_API_KEY || ''
    ...
    try {
      ...
      if (error.name === 'AbortError' || error.name === 'TimeoutError') {
        const timeoutError = new Error(
          `Request timed out after ${duration}ms. Ollama Cloud may be slow or unavailable.`,
        )
