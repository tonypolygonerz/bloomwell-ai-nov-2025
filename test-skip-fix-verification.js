...
async function testDirectNavigation(page, resultsDir) {
  ...
  try {
    ...
    if (currentUrl.includes('/onboarding/step2')) {
      ...
      console.log('⚠️  User already has onboarding complete (expected if testing with existing user)');
...
async function ensureAuthenticated(page) {
  ...
  try {
    await page.goto('http://localhost:3000/onboarding/step2', { waitUntil: 'networkidle2', timeout: 5000 });
    const finalUrl = page.url();
